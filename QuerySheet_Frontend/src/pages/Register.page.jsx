...
export default function RegisterPage() {
  ...
  async function handleRegister(e) {
    ...
    try {
      const { firebaseCustomToken } = await registerUser(
        email.trim(),
        password,
        displayName.trim(),
      );
      ...
      if (firebaseCustomToken) {
        // Backend returned a bridge token — sign in immediately so the user
        // lands on /dashboard without a separate login step.
        await signInWithToken(firebaseCustomToken);
        // onAuthStateChanged fires → useAuth → PublicOnlyRoute redirects automatically.
      } else {
        // Fallback: backend doesn't yet return a custom token for register.
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2 bg-surface">
      <AuthBrandPanel
        headline="Start your data journey."
        subtitle="Create your account and turn raw spreadsheets into clear, actionable insights in minutes."
      />
